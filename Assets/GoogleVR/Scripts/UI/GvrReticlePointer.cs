...
public class GvrReticlePointer : MonoBehaviour {
  ...
  public float reticleGrowthSpeed = 8.0f;
    ...
    private GameObject walkControll;
  ...
  void Start() {
    ...
    CreateReticleVertices();
        gazeTime = 0f;
        walkControll = GameObject.FindGameObjectWithTag("Player");
        waitBar.value = 0;
  }
  ...
  void Update() {
    ...
    reticlePointerImpl.UpdateDiameters();
        if (reticlePointerImpl.onTarget == true)
        {
            
            gazeTime += Time.deltaTime;
            updateBar();
            if (gazeTime >= triggerWait)
            {
                waitBar.value = 0;
                updateBar();
                walkControll.GetComponent<autowalk>().setMoveForward();
            }
        }
        else {
            gazeTime = 0;
